...
public class NotificationList : MonoBehaviour {
	...
	void OnGUI(){
		for (int i = notifications.Count - 1; i >= 0; i--){
			...
			GUI.DrawTexture(new Rect(Screen.width / 2 - notifitexture.width / 2, noti.y, notifitexture.width, notifitexture.height), notifitexture);
			
			GUI.color = new Color(0, 0, 0, colour.a);
			
			string display = noti.message;
			Vector2 size = GUI.skin.label.CalcSize(new GUIContent(display));
			GUI.Label(new Rect(Screen.width / 2 - size.x / 2, noti.y + notifitexture.height / 2 - size.y / 2, size.x, size.y), display);
			GUI.color = orig;
		}
	}
